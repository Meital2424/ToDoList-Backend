...
// app.MapPost("/tasks", async ([FromServices] ToDoDbContext db, Item item) =>
// {
//     db.Items.Add(item);
//     await db.SaveChangesAsync();
...
// app.MapPut("/tasks/{id}", async ([FromServices]ToDoDbContext db, int id, Item updatedItem) =>
// {
...
if (app.Environment.IsDevelopment())
{
    app.UseDeveloperExceptionPage(); // הוספת שורה זו
    app.UseSwagger();
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "Todo API v1");
